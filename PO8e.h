...
#include <stdint.h>
    ...
    /* strange way to detect Matlab triggered cl.exe builds */
    #ifdef WIN32
        #ifndef __cplusplus
            typedef int bool;
            enum
...
public:
    ...
    /* Specify the target card by index.  The index will be consistent
       ...
       can change.  The optional port is physical port on the PO8e 
       card.  For singal port cards the only valid value is 0, for
       dual port cards either 0 or 1 can be used.
     */
    static PO8e* connectToCard(unsigned int cardIndex = 0, unsigned int cardPort = 0);
    ...
    /* poll PO8e card in same interval regardless of card buffer status
 *     by default, interval is longer when no data is available on the card */
    void setFastModeEnabled();
...
// TODO: they must call something to clear the stop condition!
    ...
    /* Copy the data buffered for an individual channel.  Note that
       ...
       is large enough to hold nSamples * dataSampleSize() bytes.  The
       offsets parameter is optional and works just like the one for
       readBlock described below. */
    ...
    /* Read a block of data and releases the data buffers it occupied
       ...
       nSamples * numChannels() * dataSampleSize() bytes.  The
       optional offsets array should be nSamples long and will be
       populated with the data offset of each block.  This allows a
       user to detect if the buffer on the RZ unit has overflowed. */
...
extern "C"
{
    ...
    PO8E_API PO8eCard connectToCard(unsigned int cardIndex, unsigned int cardPort);
    ...
    PO8E_API void setFastModeEnabled(PO8eCard card);
    ...
    PO8E_API int readChannel(PO8eCard card, int chanIndex, void *buffer, int nSamples, int64_t *offsets);
    PO8E_API int readBlock(PO8eCard card, void *buffer, int nSamples, int64_t *offsets);
